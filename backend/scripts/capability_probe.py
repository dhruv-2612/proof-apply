...
from app.models import Strict, now
...
class Probe(Strict):
    ...
    test_count: int
try:
    response=client.models.generate_content(model=args.model,contents='Fictional fixture: a candidate wrote 18 Vitest tests. Extract skill and test_count.',config=types.GenerateContentConfig(response_mime_type='application/json',response_json_schema=Probe.model_json_schema(),max_output_tokens=500))
    result=Probe.model_validate_json(response.text)
    report['structured_output']='passed' if result.test_count==18 else 'failed'
...
if report['structured_output']=='passed':
    try:
        ...
        contents=[types.Content(role='user',parts=[types.Part.from_text(text=prompt)]),response.candidates[0].content,types.Content(role='tool',parts=[types.Part.from_function_response(name='read_fixture',response={'count':18})])]
        follow=client.models.generate_content(model=args.model,contents=contents,config=types.GenerateContentConfig(max_output_tokens=500))
        report['custom_function']='passed' if '18' in (follow.text or '') else 'failed'
    except Exception as exc: report['custom_function']='failed: '+str(getattr(exc,'code',type(exc).__name__))
    try:
        response=client.models.generate_content(model=args.model,contents='Use URL Context to read https://www.google.com/about/ and state one product fact. No personal information.',config=types.GenerateContentConfig(tools=[types.Tool(url_context=types.UrlContext())],max_output_tokens=1000))
