...
def create_app(config=settings, directory=None, provider_factory=None):
    ...
    def capabilities():
        return {'mode':config.llm_mode,'model':config.gemini_model or None,'live_processing_enabled':bool(config.live_ready()),'research_options':['provided_kb'],'temporary_storage':True,'synthetic_only_live':True,'prompt_version':'v1','sdk_version':__import__('importlib.metadata',fromlist=['version']).version('google-genai'),'url_context_enabled':config.feature_ready('url_context'),'limitations':['Stitch references unavailable; temporary design.','Personal documents are local/offline only.']}
